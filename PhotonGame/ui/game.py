...
class GameScreen(QtWidgets.QWidget):
    ...
    def reset_to_idle(self, default_secs: int = 6 * 60):
        ...
        # Optional: clear HUD between matches
        # self.feed.clear()
    ...
    def beginCountdownThenStart(self, secs: int = DEFAULT_PREGAME_SECS, game_length_secs: int = DEFAULT_GAME_SECS):
        ...
        self._countdown_timer.start()  # 200 ms interval (set in _install_timers)
    ...
    def _tick_countdown(self):
        ...
        if remaining != self._countdown_secs:
            ...
            if (remaining == 17) and (not self._music_started):
                try:
                    audio.stop_music()  # stop anything stray
                except Exception:
                    pass
                try:
