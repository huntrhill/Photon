...
class EntryScreen(QtWidgets.QWidget):
	...
	def __init__(self, parent=None):
		...
		red_lay = QtWidgets.QVBoxLayout(red_box)     # attach layout to the box
		red_lay.addWidget(self.red_list)
		rosters.addWidget(red_box)

		# ---- Green box ----
		green_box = QtWidgets.QGroupBox("Green", parent=self)
		green_lay = QtWidgets.QVBoxLayout(green_box) # attach layout to the box
		green_lay.addWidget(self.green_list)
...
class GameScreen(QtWidgets.QWidget):
	...
	def __init__(self, parent=None):
		...
		# Make ID tight, Codename stretch, Score tight (optional but nicer)
		for t in (self.red_table, self.green_table):
		...
		# ---- Red group ----
		red_box = QtWidgets.QGroupBox("Red", parent=self)
		red_lay = QtWidgets.QVBoxLayout(red_box)
		red_lay.addWidget(self.red_table)
		bottom.addWidget(red_box)
		
		# ---- Green group ----
		green_box = QtWidgets.QGroupBox("Green", parent=self)
		green_lay = QtWidgets.QVBoxLayout(green_box)
		green_lay.addWidget(self.green_table)
		bottom.addWidget(green_box)
		
		# Make boxes share space evenly
		bottom.setStretchFactor(red_box, 1)
		bottom.setStretchFactor(green_box, 1)
